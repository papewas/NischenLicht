...
$(function() {
	...
	function init_template(){
        ...
        setTimeout(function(){
            ...
            $('#'+menuActive).addClass('menu-item-active');
        },1500);
        ...
        if(menuFooter.length){
            ...
            pageContent.css('min-height', windowSize);
            pageContent.css('padding-bottom','60px');
        }
        
        if($('#footer-menu .active-nav').length){
            $('#footer-menu .active-nav').append('<strong></strong>')
            $('body').addClass('is-footer-menu')
        }
        
        //Dark Mode Settings
        function createCookie(e, t, n) {if (n) {var o = new Date;o.setTime(o.getTime() + 480000 * n * 60 * 60 * 1e3);var r = "; expires=" + o.toGMTString()} else var r = "";document.cookie = e + "=" + t + r + "; path=/"}
        ...
        $('[data-toggle-theme]').off('click').on('click',function(e){
            ...
            if($('body').hasClass('theme-light')){eraseCookie('Appeca_dark_mode'); createCookie('Appeca_light_mode', true, 1);} 
            if($('body').hasClass('theme-dark')){eraseCookie('Appeca_light_mode'); createCookie('Appeca_dark_mode', true, 1);}
            //e.preventDefault;
            return false;
        });      
        
        if (readCookie('Appeca_dark_mode')) {$('body').removeClass('theme-light').addClass('theme-dark');}
        if (readCookie('Appeca_light_mode')) {$('body').removeClass('theme-dark').addClass('theme-light');}
		...
		setTimeout(function(){
            ...
            $('.task-slider').owlCarousel({loop:true, margin:0, nav:false, lazyLoad:true, items:1, autoplay: true, autoplayTimeout:4000});		
            $('.user-list-slider').owlCarousel({loop:false, margin:30, nav:false, lazyLoad:true, dots:false, items:1, autoplay: false});		
            $('.cta-slider').owlCarousel({loop:true, margin:0, nav:false, lazyLoad:true, items:1, autoplay: true, autoplayTimeout:4000});		
			...
			$('.double-slider').owlCarousel({loop:true, margin:40, nav:false, lazyLoad:true, items:2, autoplay: true, autoplayTimeout:4000});		
			...
			$('.next-slide-user, .next-slide-custom').on('click',function(){$('.owl-carousel').trigger('next.owl.carousel');});		
			$('.prev-slide-user, .prev-slide-custom').on('click',function(){$('.owl-carousel').trigger('prev.owl.carousel');});		
		},0);
        ...
        function fullPage(){
            ...
            $('.caption').each(function(){
                ...
                var windowNotchHeight = $(window).height()+45;
                ...
                if(captionHeight === "cover-header"){
                    if($('body').hasClass('is-on-homescreen')){
                        $(this).css('height', windowNotchHeight);
                        $('.map-full').css('height', windowNotchHeight);
                    } else {
...
//            var share_title = document.title;
            var share_link = 'https://preview.enableds.com?theme=Appeca';
            var share_title = 'Appeca | Mobile Template & PWA';
        ...
        jQuery(document).ready(function(e) {
            function t(t, n) {
                ...
                e.post(e("#" + t).attr("action"), r, function(n) {
                    e("#" + t).hide();
                    e("#formSuccessMessageWrap").removeClass('disabled');
                })
            }

            function n(n, r) {
                e(".formValidationError").addClass('disabled');
                ...
                e("#" + n + " .requiredField").each(function(i) {
                    if (e(this).val() == "" || e(this).val() == e(this).attr("data-dummy")) {
                        ...
                        e("#" + e(this).attr("id") + "Error").removeClass('disabled');
                        return false
                    }
                    if (e(this).hasClass("requiredEmailField")) {
                        ...
                        if (!s.test(e(o).val())) {
                            ...
                            e(o + "Error2").removeClass('disabled');
                            return false
                        }
                    }
            ...
            e("#formSuccessMessageWrap").addClass('disabled');
            e(".formValidationError").addClass('disabled');
        ...
        function activate_search(){
            search.on('keyup', function() {
                ...
                if ( searchVal != '' ) {
                    $(this).parent().parent().find('.search-results').removeClass('disabled');
                    $(this).parent().parent().find('[data-filter-item]').addClass('disabled');
                    $('.search-clear').removeClass('disabled');
                    $('.search-no-results').addClass('disabled');
                    $(this).parent().parent().find('[data-filter-item][data-filter-name*="' + searchVal.toLowerCase() + '"]').removeClass('disabled');
                } else {
                    $(this).parent().parent().find('.search-results').addClass('disabled');
                    $(this).parent().parent().find('[data-filter-item]').removeClass('disabled');
                    $('.search-clear').removeClass('disabled');
                }
            
                var searchTotalItems = $('.search-results a').length;                
                var search_results_error = $('.search-no-results');
                var search_results_active = $('.search-results').find('.disabled');
        ...
        if(footer.length){footer.load(footerLoad)};
        ...
        function highlight_colors(){
            ...
            if (data.indexOf(bodyColor) > -1) {
                ...
                if(!generatedHighlight.length){
                    ...
                    $('.generated-highlight').append(highlightColor, highlightBg, highlightNav, highlightGradient, highlightNavFixed, highlightBorder, highlightHeaderTabs);
                }
            }
        }      
        highlight_colors();
		...
		var options = {
			...
			onStart: {
				...
				$container.addClass('is-exiting');// Add your CSS animation reversing class
                    ...
                    $('.loader-main .loader-top, .loader-main .loader-bottom').remove();
					return false;
				}
			},
			onReady: {
